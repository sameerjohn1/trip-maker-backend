...
// Legacy seller endpoints now operate for regular users who own the resource.
// Ownership checks in their controllers remain the authorization boundary.
export const requireSellerActive = (req, res, next) => {
  if (!req.user || !["USER", "ADMIN"].includes(req.user.role) || req.user.status !== "ACTIVE") {
    return next(new AppError("An active user account is required", 403));
  }
  next();
};
